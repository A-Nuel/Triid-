  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button 
          onClick={() => {
            const profileId = Array.isArray(profile?.artisan_profiles) 
              ? profile.artisan_profiles[0]?.id 
              : profile?.artisan_profiles?.id;
            if (profileId) {
              navigate(`/resident/artisan/${profileId}`);
            }
          }}
          disabled={!profile}
          className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm self-start md:self-auto"
        >
